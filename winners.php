    ...
    if (isset($_POST['winners'])) {
        foreach ($_POST['winners'] as $selects) {
            ...
            if (isset($selects['winner'])) {
                /*update game outcome for each game*/
                $query="UPDATE games SET outcome=".$selects['winner']." WHERE id='".$selects['gameid']."'"; 
                ...
                while ($row=pg_fetch_assoc($result)) {
                    ...
                    if ($outcome==$pick) {
                        /*update pick_result=1 for each game where result matched outcome*/
                        $pickresultquery="UPDATE picks SET pick_result=1 WHERE game_id='".$selects['gameid']."' AND id='$id'"; 
                        $pickresult=pg_exec($cxn,$pickresultquery) or die ("Could not execute pick pick_result=1 update query");
                    } else {
                        /*update pick_result=0 for each game where result didn't match outcome*/
                        $pickresultquery="UPDATE picks SET pick_result=0 WHERE game_id='".$selects['gameid']."' AND id='$id'"; 
            ...
            if ($_POST['tbgame']==$selects['gameid']) {
                /*update tiebreaker points for tiebreaker game*/
                $query="UPDATE games SET tiebreakerpts=".$_POST['tiebreakerpts']." WHERE id='".$selects['gameid']."'"; 
                $result=pg_exec($cxn,$query) or die ("Could not execute tiebreaker game points update query");
            } else {
                /*update tiebreaker points to 0 for non-tiebreaker game*/
                $query="UPDATE games SET tiebreakerpts=0 WHERE id='".$selects['gameid']."'";
                $result=pg_exec($cxn,$query) or die ("Could not execute non-tiebreaker game update query");

            }
        } //end foreach ($_POST['winners']
        ...
        while ($row=pg_fetch_assoc($result)) {
            ...
            while ($pickrow=pg_fetch_assoc($pickresult)) {
                ...
                if ($pick_result==1) {
                    ...
                    $playerwrongpickcounter=$playerwrongpickcounter+1;
                }
